...
//     // задаем условие цикла. Прибавляем единицу для того, чтобы избежать бесконечного цикла, т.к. если всегда проверять по
// найденной позиции то мы навсегда остановимся на первом пробеле, ведь в отличие от 2 задачи прбел у нас не удаляется
//     pos = str.indexOf(' ', pos + 1); // ищем индекс, под которым расположен пробел
//     str =
//       str.slice(0, pos + 1) + str[pos + 1].toUpperCase() + str.slice(pos + 2); // первый slice делит строку до первой буквы следующего
//  слова, прибавляется к ней первую букву слова в верхнем регистре и оставшуюся строку, после этой буквы.
//   }
//   return str;
// }
...
//     if (str[i].toUpperCase() !== str[i].toLowerCase()) { // проверяем буква ли текущий символ.
// Только буквы при сравнении не будут равны в верхнем и нижнем регистре, т.к. у символов и цифр нет
// других регистров и их код не меняется
...
//     // цикл будет выполняться до тех пор, пока количество слов будет меньше требуемуму количеству слов n и пока не закончатся слова в
// строке(т.е. если хоть одно из условий станет false, то цикл прекратится). Т.к. слова разделены пробелмаи, то мы снова будем ориентироваться
// по проблеам. В условии добавляем единицу, чтобы исключить из рассмотрения расмотренный на прошлой итерации пробел
//     count += 1;
